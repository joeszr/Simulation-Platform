///@file PathLossUrbanMicroLOS_Highfreq.cpp
///@brief  PathLossUrbanMicroLOS_Highfreq类定义
///@author wangfei
...
using namespace cm;
///构造函数
RIS2MSPathLossUrbanMicroLOS_ModeB::RIS2MSPathLossUrbanMicroLOS_ModeB( void ){
}
///析构函数
RIS2MSPathLossUrbanMicroLOS_ModeB::~RIS2MSPathLossUrbanMicroLOS_ModeB( void ){
}
///计算DB值
double RIS2MSPathLossUrbanMicroLOS_ModeB::Db( double _dDisM_3D , double _dUEHeightM){
    ...
    if (d2DisM >= 10 && d2DisM <= dEffectBreakPointM) {
        ...
        //high freq
        const static double a = 32.4
                  + 20 * std::log10(P::s().FX.DRadioFrequencyMHz_Macro / 1e3);
        return -1 * (a + 21.0 * std::log10(_dDisM_3D));
        ...
        //high freq
        const static double b = 32.4
                  + 20 * std::log10(P::s().FX.DRadioFrequencyMHz_Macro / 1e3)
                 - 9.5 * std::log10(
                    pow(dEffectBreakPointM, 2)
                    + pow((dEffectNBHeightM - dEffectUEHeightM), 2)); //36.900
