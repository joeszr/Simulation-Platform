...
using namespace cm;
///构造函数
RIS2MSPathLossUrbanMacroOutdoorLOS::RIS2MSPathLossUrbanMacroOutdoorLOS( void ){
}
///析构函数
RIS2MSPathLossUrbanMacroOutdoorLOS::~RIS2MSPathLossUrbanMacroOutdoorLOS( void ){
}
///计算DB值
double RIS2MSPathLossUrbanMacroOutdoorLOS::Db( double _dDisM_3D, double _dUEHeightM){
//    assert( Parameters.Instance().Macro.LINK.DRadioFrequencyMHz == 2000 );
//    //F=2GHz
//    static double a = 103.8 + 20.9 * std::log10( _dDisM );
//    return -1 * a;
...
//    assert(P::s().RX.DAntennaHeightM == 1.5);
    ...
    if (xUniform_channel() <= 1.0 / (1.0 + C)) {
        ...
        int MaxBuildingFloorNum = (_dUEHeightM - 1.5) / 3.0;
    ...
    if (d2DisM < 10.0)
        return 0.0;
    if (d2DisM > 0 &&d2DisM <= dEffectBreakPointM) {
        //low frequency
        ...
        double b  = 28.0 + 20 * std::log10(P::s().FX.DRadioFrequencyMHz_Macro / 1e3)
                 - 9 * std::log10(
                    ...
                    + pow((Parameters::Instance().RIS.DAntennaHeightM - _dUEHeightM), 2)); //36.900
