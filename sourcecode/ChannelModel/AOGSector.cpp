...
AOGSector::AOGSector(double _d3DBBeamWidthHDeg, double _d3DBBeamWidthVDeg, double _dHBackLossDB, double _dVBackLossDB, double _dABackLossDB, double _dDownTiltDeg) {
...
double AOGSector::Db(double _dAngleRad, double _dDownTiltRad) {
    double dAngle = ConvergeAngle(_dAngleRad);
    double result1 
        = std::max(
            -12 * pow((dAngle * 180 / M_PI) / m_d3DBBeamWidthHDeg, 2.0), 
            m_dHBackLossDB);
    ///适用于CaseC
//    double result2 = std::max(-12 * pow(((DEG2RAD(m_dDownTiltDeg) - _dDownTiltRad) * 180 / M_PI) / m_d3DBBeamWidthVDeg, 2.0), m_dVBackLossDB);
    ...
    double result2 
        = std::max(-12 * pow((dDownTiltRad * 180 / M_PI - 90) / m_d3DBBeamWidthVDeg, 2.0), 
            m_dVBackLossDB);
...
double AOGSector::Linear(double _dAngleRad, double _dDownTiltRad) {
