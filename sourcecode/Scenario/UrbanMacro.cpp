...
void UrbanMacro::SetMSPara(MS* _pms){
...
std::shared_ptr<PathLoss> UrbanMacro::GetPathLossPtr(Tx& _tx, Rx& _rx, bool _bIsLOS){
...
std::shared_ptr<PathLoss> UrbanMacro::GetPathLossPtr(double _dInDoorDisM, bool _bIsLOS,
                                                     int _iIslowloss, double _dPenetrationSFdb){
    std::shared_ptr<PathLoss> result;
    result = std::shared_ptr<PathLoss > (
            new PathLossUrbanMacroO2I_Highfreq(_dInDoorDisM, _bIsLOS,
                                      _iIslowloss, _dPenetrationSFdb));
    return result;
}

double UrbanMacro::GenPos2Din(std::pair<int, int> pos) {
...
bool UrbanMacro::DecideLOS(double _dDisM_2D, bool _bIsMacro2UE, double _h_UT) {
...
void UrbanMacro::ReadMapPos(Point& _tx, Rx& _rx, BasicChannelState* const bcs){
    itpp::mat r, y, x(7, 1);
...
void UrbanMacro::SetSCSPara(BasicChannelState* const bcs, double _dTxHeight, double _dRxHeight){
...
void UrbanMacro::InitializeAOD(SpaceChannelState* pSCS){
...
void UrbanMacro::InitializeAOA(SpaceChannelState* pSCS){
    double dMaxPathPower = 0;
    BasicChannelState* pBCS = pSCS->m_pBCS;
    for (int i = 0; i < pBCS->m_iNumOfPath; ++i) {
        if (pSCS->m_vPath[i].m_Power > dMaxPathPower) {
...
void UrbanMacro::InitializeEOD(SpaceChannelState* pSCS){
    double dMaxPathPower = 0;
    BasicChannelState* pBCS = pSCS->m_pBCS;
    for (int i = 0; i < pBCS->m_iNumOfPath; ++i) {
        if (pSCS->m_vPath[i].m_Power > dMaxPathPower) {
...
void UrbanMacro::InitializeEOA(SpaceChannelState* pSCS){
    double dMaxPathPower = 0;
    BasicChannelState* pBCS = pSCS->m_pBCS;
    for (int i = 0; i < pBCS->m_iNumOfPath; ++i) {
        if (pSCS->m_vPath[i].m_Power > dMaxPathPower) {
