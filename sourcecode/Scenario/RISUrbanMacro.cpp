...
#include "../ChannelModel/PathLossUrbanMacroLOS_Highfreq.h"
#include "../RIS/RIS2MSPathLossUrbanMacroIndoor.h"
#include "../Parameters/Parameters.h"
#include"PathState.h"
using namespace cm;

RISUrbanMacro::RISUrbanMacro(){
...
void RISUrbanMacro::SetMSPara(MS* _pms){
...
std::shared_ptr<PathLoss> RISUrbanMacro::GetPathLossPtr(Tx& _tx, Rx& _rx, bool _bIsLOS){
    std::shared_ptr<PathLoss> result;
    static std::shared_ptr<PathLoss> pPLUMaLOS
            = std::shared_ptr<PathLoss > (new PathLossUrbanMacroLOS_Highfreq());//这里有疑问
...
std::shared_ptr<PathLoss> RISUrbanMacro::GetPathLossPtr(double _dInDoorDisM, bool _bIsLOS,
                                                     int _iIslowloss, double _dPenetrationSFdb){
    std::shared_ptr<PathLoss> result;
    result = std::shared_ptr<PathLoss > (
            new RIS2MSPathLossUrbanMacroIndoor(_dInDoorDisM, _bIsLOS,
                                      _iIslowloss, _dPenetrationSFdb));

    return result;
}

double RISUrbanMacro::GenPos2Din(std::pair<int, int> pos) {
...
bool RISUrbanMacro::DecideLOS(double _dDisM_2D, bool _bIsMacro2UE, double _h_UT) {
    double dPossLOS = cm::CalcPossLOS_for_UMA_5G_ITU(_dDisM_2D, _h_UT);
    bool isLOS = xUniform_channel() < dPossLOS;

    return isLOS;
}

void RISUrbanMacro::ReadMapPos(Point& _tx, Rx& _rx, BasicChannelState* const bcs){
    itpp::mat r, y, x(7, 1);
...
void RISUrbanMacro::SetSCSPara(BasicChannelState* const bcs, double _dTxHeight, double _dRxHeight){
...
void RISUrbanMacro::InitializeAOD(SpaceChannelState* pSCS){
...
void RISUrbanMacro::InitializeAOA(SpaceChannelState* pSCS){
    double dMaxPathPower = 0;
    BasicChannelState* pBCS = pSCS->m_pBCS;
    for (int i = 0; i < pBCS->m_iNumOfPath; ++i) {
        if (pSCS->m_vPath[i].m_Power > dMaxPathPower) {
...
void RISUrbanMacro::InitializeEOD(SpaceChannelState* pSCS){
    double dMaxPathPower = 0;
    BasicChannelState* pBCS = pSCS->m_pBCS;
    for (int i = 0; i < pBCS->m_iNumOfPath; ++i) {
        if (pSCS->m_vPath[i].m_Power > dMaxPathPower) {
...
void RISUrbanMacro::InitializeEOA(SpaceChannelState* pSCS){
    double dMaxPathPower = 0;
    BasicChannelState* pBCS = pSCS->m_pBCS;
    for (int i = 0; i < pBCS->m_iNumOfPath; ++i) {
        if (pSCS->m_vPath[i].m_Power > dMaxPathPower) {
