...
FTPModel3::FTPModel3(double _dPacketSize, double _dMacroLamda):mMSID3ReadTimeMsec(Parameters::Instance().BASIC.ITotalMSNum),mMSID3PacketQueueDL(Parameters::Instance().BASIC.ITotalMSNum) {
    m_dMacroLamda = _dMacroLamda;
    m_dPacketSize = _dPacketSize;
}


void FTPModel3::WorkSlot(){
    ...
    for(BTSID btsid = BTSID::Begin(); btsid <= BTSID::End(); ++btsid) {
        ...
        //检查每个UE在BTS缓存里面的包队列，如果队列为空(空只能说明调度了，但是不定正确接收到了)，就为该UE产生新的包
        int iMSNum = static_cast<int>(vMSID.size());
        if (iMSNum <= 0) continue;
        int iPacketNum = random.xPossion_DL(dLamda);
        if (iMSNum < iPacketNum) {
            bts.SetFTPPacketLostNum(iPacketNum-iMSNum);
            iPacketNum = iMSNum;
        }
        for(int i=0;i<iPacketNum;++i){
            ...
            if(iTime == 1){
                ...
                mMSID3ReadTimeMsec[msid.ToInt()] = iTime + iReadTime;
            }
            if(!mMSID3PacketQueueDL[msid.ToInt()].empty()){
                assert(static_cast<int>(mMSID3PacketQueueDL[msid.ToInt()].size()) == 1);
                if( (bts.GetMSTxBufferBTS()[msid.ToInt()].GetDLPacketNum() == 0) && (mMSID3PacketQueueDL[msid.ToInt()][0]->IsCompleted())){
                    ///产生一个ReadTime
                    mMSID3PacketQueueDL[msid.ToInt()].pop_front();
                    int iReadTime = (int)(random.xExponent(dLamda) * 1000);
                    mMSID3ReadTimeMsec[msid.ToInt()] = iTime + iReadTime;
                }
            }
            ...
            if(mMSID3ReadTimeMsec[msid.ToInt()] == iTime){
                ///ReadTime到期后产生一个包
                ...
                mMSID3PacketQueueDL[msid.ToInt()].push_back(pPacket);
...
void FTPModel3::OutputTrafficInfo() {
